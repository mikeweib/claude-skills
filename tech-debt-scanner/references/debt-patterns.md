...
- **God class**：单文件超 1000 行的 Service/Controller
- **长方法**：超 50 行的业务方法
...
重复代码是量化技术债的关键指标，通常高的重复代码率意味着：

- **复制粘贴式开发**：bug 修复需要同步更新多处，容易遗漏
- **缺乏抽象**：相同逻辑没有提取为公共函数/模块
- **代码膨胀**：增加维护负担和构建时间

| 检测类型 | 阈值 | 健康标准 |
|----------|------|---------|
| 完全重复文件 (MD5) | 任意 >50 字节 | 0 对 |
| 复制粘贴块 | >=6 行连续相同 | 0 块 |
| 高相似度文件 | >80% 行共享率 | 0 对 |
| 文件命名模式 | `_copy`, `_v2`, `(1)` | 0 个 |

**常见误报场景：**
- 自动生成的代码（protobuf、gRPC stub）→ 已在排除规则中
- 模板/脚手架代码 → 检查是否应排除生成目录
- 接口实现间的样板代码 → 需人工判断是否为合理重复
